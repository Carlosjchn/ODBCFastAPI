...
@router.post("/create", summary="Crear un nuevo usuario en la base de datos")
async def create_user(
    ...
    3. **UserMethods.**`insert_user(user_data)` -> Procesa la solicitud y ejecuta la consulta para insertar el user
    ---
    ### Parámetros:\n
...
async def delete_user_router(id_usuario: int = None, nombre: str = None):
    ...
    ### Parámetros:\n
    - **id_usuario** (int, opcional): ID del usuario que se desea eliminar.\n
    - **nombre** (str, opcional): Nombre del usuario que se desea eliminar.\n
    ---
    ### Returns:\n
...
@router.put("/updateUser", summary="Actualizar los datos de un usuario")
